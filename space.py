...
def build_model_cem(height, width, channels, actions):
    model = Sequential()
...
def build_agent_cem(model, actions):
    ...
    cema = CEMAgent(model=model,memory=memory, nb_actions=actions, nb_steps_warmup=1000000, train_interval=10000)
...
# s = dqn.fit(env, nb_steps=100000, visualize=True, verbose=2)
# f = open('score_dqn_train.csv','w')
# for i in s.history['episode_reward']:
#     f.write(str(i))
#     f.write('\n')
# f.close()
...
model = build_model_cem(height, width, channels, actions)


cema = build_agent_cem(model, actions)
cema.compile()
# cema.fit(env, nb_steps=10000, visualize=True)
...
scores = cema.test(env, nb_episodes=50, visualize=True, verbose=2)
print(scores.history['episode_reward'])
